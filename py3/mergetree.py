...
class MergeTree(object):
    ...
    def build_merge_tree(self, X, periodic=False, return_graph=False, 
        ...
        while not it.finished:
            ...
            if it[0]:
                ...
                if periodic:
                    behind = list(map(lambda x, y: x + y, it.multi_index, (0, -1)))
                ...
                below = list(map(lambda x, y: x + y, it.multi_index, (-1, 0)))
            ...
            if it.finished or it.multi_index[0] != current_level:
                ...
                level_coords = [x for x in list(self.uf.objects_to_num.keys()) if x[0] == current_level]
    ...
    def build_merge_tree_3d(self, X, periodic=False, return_graph=False, iter_dim=0, flip=False, **kwargs):
        ...
        while not it.finished:
            ...
            if it[0]:
                ...
                if periodic:
                    behind = list(map(lambda x, y: x + y, it.multi_index, (0, 0, -1)))
                    behind = (behind[0], behind[1], behind[2] % X.shape[2])
                    if X[tuple(behind)]:
                        self.uf.union(it.multi_index, tuple(behind), elder_rule=False)

                    side = list(map(lambda a, b: a + b, it.multi_index, (0, -1, 0)))
                ...
                below = list(map(lambda x, y: x + y, it.multi_index, (-1, 0, 0)))
                if not any(n < 0 for n in below) and X[tuple(below)]:
    ...
    def mergers_per_level(self):
        ...
        """
        merge_levels = [d[0] + 1 for d in list(self.mergers.values())]
...
def update_parent_pointers(tree):
    ...
    if len(diff) == 0:
        return
    for n in diff:
        list(map(tree.uf.find,
            [tree.uf.num_to_objects[k] for k, v in list(tree.uf.parent_pointers.items())
             if v == n]))
...
def calculate_wsquared_level(X, tree):
    ...
    for l in levels:
        level_cells = {k: tree.uf.parent_pointers[v]
                       for k, v in list(tree.uf.objects_to_num.items()) if k[0] == l}
        for k, v in list(level_cells.items()):
            try:
...
def calculate_variables_level(W, T, tree):
    ...
    for l in levels:
        level_cells = {k: tree.uf.parent_pointers[v]
                       for k, v in list(tree.uf.objects_to_num.items()) if k[0] == l}
        counts = Counter(list(level_cells.values()))
    ...
    w_squared = {k: np.zeros(n_levels) for k in list(tree.uf.num_weights.keys())}
    wt = {k: np.zeros(n_levels) for k in list(tree.uf.num_weights.keys())}
    sizes = {k: np.zeros(n_levels) for k in list(tree.uf.num_weights.keys())}
    ...
    for k, v in list(sizes_level.items()):
        if k in list(sizes.keys()):
            sizes[k] += v
        else:
            sizes[update_map[k]] += v
    for k, v in list(w_squared_level.items()):
        if k in list(w_squared.keys()):
            w_squared[k] += v
        else:
            w_squared[update_map[k]] += v
    for k, v in list(wt_level.items()):
        if k in list(wt.keys()):
            wt[k] += v
        else:
            wt[update_map[k]] += v
...
def calculate_sizes_level(X, tree):
    ...
    diff = A - B
    update_map = {k: find_root(k, tree, B) for k in diff}
    components_sizes = {k: np.zeros(n_levels) for k in list(tree.uf.num_weights.keys())}
    for k, v in list(sizes.items()):
        if k in list(components_sizes.keys()):
